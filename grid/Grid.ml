...
type 'a node = Leaf of 'a | Quad of 'a node * 'a node * 'a node * 'a node
[@@deriving sexp, bin_io]

type sel = Nw | Ne | Sw | Se

type 'a t = {side: int; root: 'a node} [@@deriving sexp, bin_io]
...
let get x y t =
  ...
  let rec get_from_node x y side node =
    ...
    | Quad (nw, ne, sw, se) -> (
        ...
        | false, false ->
            get_from_node x y half nw
        | true, false ->
            get_from_node (x - half) y half ne
        | false, true ->
            get_from_node x (y - half) half sw
        | true, true ->
            get_from_node (x - half) (y - half) half se )
  in
...
let rec get_path path node =
  ...
  | _ :: _, (Leaf _ as leaf) ->
      leaf
  | Nw :: path_rest, Quad (nw, _, _, _) ->
      get_path path_rest nw
  | Ne :: path_rest, Quad (_, ne, _, _) ->
      get_path path_rest ne
  | Sw :: path_rest, Quad (_, _, sw, _) ->
      get_path path_rest sw
  | Se :: path_rest, Quad (_, _, _, se) ->
      get_path path_rest se
...
let rec force_set_node x y new_v side node =
  ...
  | Leaf _ as l -> (
    match (x >= half, y >= half) with
    | false, false ->
        let new_nw = force_set_node x y new_v half l in
        Quad (new_nw, l, l, l)
    | true, false ->
        let new_ne = force_set_node (x - half) y new_v half l in
        Quad (l, new_ne, l, l)
    | false, true ->
        let new_sw = force_set_node x (y - half) new_v half l in
        Quad (l, l, new_sw, l)
    | true, true ->
        let new_se = force_set_node (x - half) (y - half) new_v half l in
        Quad (l, l, l, new_se) )
  | Quad (nw, ne, sw, se) -> (
    match (x >= half, y >= half) with
    | false, false ->
        let new_nw = force_set_node x y new_v half nw in
        Quad (new_nw, ne, sw, se)
    | true, false ->
        let new_ne = force_set_node (x - half) y new_v half ne in
        Quad (nw, new_ne, sw, se)
    | false, true ->
        let new_sw = force_set_node x (y - half) new_v half sw in
        Quad (nw, ne, new_sw, se)
    | true, true ->
        let new_se = force_set_node (x - half) (y - half) new_v half se in
        Quad (nw, ne, sw, new_se) )
...
let rec force_set_path path new_v node =
  ...
  | Nw :: path_rest, (Leaf _ as l) ->
      let nw = force_set_path path_rest new_v l in
      Quad (nw, l, l, l)
  | Ne :: path_rest, (Leaf _ as l) ->
      let ne = force_set_path path_rest new_v l in
      Quad (l, ne, l, l)
  | Sw :: path_rest, (Leaf _ as l) ->
      let sw = force_set_path path_rest new_v l in
      Quad (l, l, sw, l)
  | Se :: path_rest, (Leaf _ as l) ->
      let se = force_set_path path_rest new_v l in
      Quad (l, l, l, se)
  | Nw :: path_rest, Quad (nw, ne, sw, se) ->
      let new_nw = force_set_path path_rest new_v nw in
      Quad (new_nw, ne, sw, se)
  | Ne :: path_rest, Quad (nw, ne, sw, se) ->
      let new_ne = force_set_path path_rest new_v ne in
      Quad (nw, new_ne, sw, se)
  | Sw :: path_rest, Quad (nw, ne, sw, se) ->
      let new_sw = force_set_path path_rest new_v sw in
      Quad (nw, ne, new_sw, se)
  | Se :: path_rest, Quad (nw, ne, sw, se) ->
      let new_se = force_set_path path_rest new_v se in
      Quad (nw, ne, sw, new_se)

let make ~side init = assert_side_invariant side ; {side; root= Leaf init}
...
include Container.Make (struct
  ...
  let fold t ~init ~f =
    let rec go node accum ~side ~f =
      ...
      | Quad (nw, ne, sw, se) ->
          let half = side / 2 in
          go nw accum ~side:half ~f |> go ne ~side:half ~f
          |> go sw ~side:half ~f |> go se ~side:half ~f
    in
...
struct
  ...
  let compact_quad = function
    | (Leaf nw as l), Leaf ne, Leaf sw, Leaf se
      when Args.(nw = ne && ne = sw && sw = se) ->
        l
    | nw, ne, sw, se ->
        Quad (nw, ne, sw, se)
  ...
  let update x y ~f t =
    let rec upd_if_different x y ~f side node =
      ...
      | Leaf current_v as l ->
          ...
          if Args.(current_v = new_v) then None
          else Some (force_set_node x y new_v side l)
      | Quad (nw, ne, sw, se) -> (
          let open Option.Monad_infix in
          let half = side / 2 in
          match (x >= half, y >= half) with
          | false, false ->
              upd_if_different x y ~f half nw
              >>| fun new_nw -> compact_quad (new_nw, ne, sw, se)
          | true, false ->
              upd_if_different (x - half) y ~f half ne
              >>| fun new_ne -> compact_quad (nw, new_ne, sw, se)
          | false, true ->
              upd_if_different x (y - half) ~f half sw
              >>| fun new_sw -> compact_quad (nw, ne, new_sw, se)
          | true, true ->
              upd_if_different (x - half) (y - half) ~f half se
  ...
  let set_path path new_v t =
    let rec set_path_node path new_v node =
      ...
      | path, (Leaf _ as l) ->
          Some (force_set_path path new_v l)
      (* Otherwise walk down the tree *)
      | Nw :: path_rest, Quad (nw, ne, sw, se) ->
          set_path_node path_rest new_v nw
          >>| fun new_nw -> Quad (new_nw, ne, sw, se)
      | Ne :: path_rest, Quad (nw, ne, sw, se) ->
          set_path_node path_rest new_v ne
          >>| fun new_ne -> Quad (nw, new_ne, sw, se)
      | Sw :: path_rest, Quad (nw, ne, sw, se) ->
          set_path_node path_rest new_v sw
          >>| fun new_sw -> Quad (nw, ne, new_sw, se)
      | Se :: path_rest, Quad (nw, ne, sw, se) ->
          set_path_node path_rest new_v se
  ...
  let map ~f t =
    let rec go old_node =
      ...
      | Leaf old_v ->
          Leaf (f old_v)
      | Quad (nw, ne, sw, se) ->
          compact_quad (go nw, go ne, go sw, go se)
    in
  ...
  let zip_map ~f a b =
    ...
    let rec go a b =
      ...
      | Leaf la, Leaf lb ->
          Leaf (f la lb)
      | Leaf _, Quad (nwb, neb, swb, seb) ->
          compact_quad (go a nwb, go a neb, go a swb, go a seb)
      | Quad (nwa, nea, swa, sea), Leaf _ ->
          compact_quad (go nwa b, go nea b, go swa b, go sea b)
      | Quad (nwa, nea, swa, sea), Quad (nwb, neb, swb, seb) ->
          compact_quad (go nwa nwb, go nea neb, go swa swb, go sea seb)
    in
      ...
      | Quad (nw, ne, sw, se) -> (
          ...
          | (Leaf nw as l), Leaf ne, Leaf sw, Leaf se
            when Args.(nw = ne && ne = sw && sw = se) ->
              Some l
          | _ -> (
            match (opt_nw, opt_ne, opt_sw, opt_se) with
            | None, None, None, None ->
                None
            | _ ->
                Some (Quad (new_nw, new_ne, new_sw, new_se)) ) )
    in
  ...
  let map_of_mut ~f m =
    init ~side:(Mut.side m) (fun (x, z) -> f ~x ~z (Mut.get ~x ~z m))
end
...
let%expect_test "force_sets, scans, and counts leaves" =
  ...
  let g = g |> force_set 1 2 10 |> force_set 6 7 20 |> force_set 6 7 21 in
  print g ~f:string_of_int ;
  Printf.printf "leaves: %d\n\n" (Leafwise.length (Leafwise.T g)) ;
  [%expect
    "\n\
