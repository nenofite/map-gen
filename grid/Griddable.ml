...
module Make (T : Arg) : S with type 'a t = 'a T.t and type 'a elt = 'a T.elt =
struct
  include T

  let is_within ~x ~z t = is_within_side ~x ~z (side t)

  let set_opt ~x ~z v t = if is_within ~x ~z t then set ~x ~z v t else t

  let get_opt ~x ~z t = if is_within ~x ~z t then Some (get ~x ~z t) else None

  let update_opt ~x ~z ~f t = if is_within ~x ~z t then update ~x ~z ~f t else t
