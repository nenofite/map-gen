...
let colorize = (tile: tile): int => {
  ...
  if (tile.ocean) {
    ...
    let frac = min(1., float_of_int(tile.river) /. 2.);
    Color.blend(base, blue, frac) |> Color.int_of_color(_);
  };
};
...
let rec raindrop' =
  ...
  if (!here.ocean) {
    ...
    /* if (sediment > 0) {
         ...
         if (sediment > 0) {
           raindrop'(~grid, ~sediment, ~delta_x, ~delta_y, x, y);
         };
       } */
    | Some((dx, dy)) =>
      let (next_x, next_y) =
      ...
      /*
       if (here.river < 5) {
         ...
         let here = {
           ...here,
           elevation: here.elevation - 1,
           river: here.river + 1,
         };
         let sediment = sediment + 1;
         Grid.put(grid, x, y, here);
         raindrop'(~grid, ~sediment, ~delta_x=dx, ~delta_y=dy, next_x, next_y);
       } else {
         /* Rule 4: add to river */
         */
...
let raindrop = (grid, x, y) =>
  raindrop'(~grid, ~sediment=0, ~delta_x=0, ~delta_y=1, x, y);

let random_raindrops = (heightmap: Grid.t(tile)) => {
  let amount = heightmap.width * heightmap.height / 30;
...
let phase =
  Phase_chain.(convert(_) @> repeat(1, random_raindrops(_)) @@> finish);