...
let rec run' = (~grid_side, ~goal, ~edge_cost, ~heuristic, ~nodes, ~open_set) =>
  ...
  | (Some(current), open_set) =>
          ...
          if (Sparse_grid.is_within(grid_side, neigh_x, neigh_y)) {
            ...
            | Some(c) =>
              ...
              | Some(_) =>
                ...
                let open_set =
                  Priority_queue.insert(open_set, new_node.f_score, new_node);
                (nodes, open_set);
              };
            };
          } else {
    ...
    run'(~grid_side, ~goal, ~edge_cost, ~heuristic, ~nodes, ~open_set);
  };
/**
