...
let grid_of_txt ~palette txt =
  let parse_line lines_rev line_txt =
    ...
    let cells_rev =
      List.fold words ~init:[] ~f:(fun cells word ->
          List.Assoc.find_exn palette ~equal:String.equal word :: cells)
    in
  ...
  let lines =
    List.rev
      (List.fold
         (String.split_lines @@ String.strip txt)
         ~init:[] ~f:parse_line)
  in
  ...
  List.iteri lines ~f:(fun z line ->
      List.iteri line ~f:(fun x cell -> Grid.Mut.set ~x ~z cell grid)) ;
  grid
