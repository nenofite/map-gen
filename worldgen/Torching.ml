...
let spread_light_into ~x ~y ~z ~level light_levels region acc = (
  ...
  let xyz = (x, y, z) in
  if not (is_solid_opt (Minecraft.Region.get_block_opt ~x ~y ~z region)) &&
...
let mark_torch_light ~x ~y ~z light_levels region = (
  let module Vi = Geometry.Vec3i in
  Grid_flood.flood_gen ~init: () ~live: [(torch_light_level, [(x, y, z)])] ~spread:(fun () ~level (x, y, z as xyz) ->
      if level > 0 then
        Vi.Table.set light_levels ~key: xyz ~data: level;
      let next_level = level - 1 in
      let next_live =
        [] |>
        (*u*) spread_light_into ~x ~y:(y + 1) ~z ~level:next_level light_levels region |>
        (*N*) spread_light_into ~x ~y ~z:(z - 1) ~level:next_level light_levels region |>
        (*E*) spread_light_into ~x:(x + 1) ~y ~z ~level:next_level light_levels region |>
        (*S*) spread_light_into ~x ~y ~z:(z + 1) ~level:next_level light_levels region |>
        (*W*) spread_light_into ~x:(x - 1) ~y ~z ~level:next_level light_levels region |>
        (*d*) spread_light_into ~x ~y:(y - 1) ~z ~level:next_level light_levels region
      in
      (), next_live
    )
)
...
let iter_with_progress ?every arr ~f = (
  ...
  let every = match every with
    | Some n -> n
    | None -> Int.max (total / 20) 1
  in
  let prog i total =
...
let illuminate ?(min_brightness = 8) ~volume region = (
  Tale.block "Illuminating" ~f:(fun () ->
      ...
      iter_with_progress ~every:100 sorted_surfaces ~f:(fun ((preference, _random), (x, y, z as xyz)) ->
          if Option.value (Vi.Table.find light_levels xyz) ~default: 0 < min_brightness then
            match Preference.apply_at ~x ~y ~z region preference with
            | Some (x, y, z) -> mark_torch_light ~x ~y ~z light_levels region
            | None -> ()
        );
    );
)
