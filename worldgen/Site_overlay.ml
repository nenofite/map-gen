open Core_kernel
type site =
  | Cavern_entrance of int [@@deriving bin_io]
...
let prepare () =
  ...
  Point_cloud.init_f ~side:(canon.side) ~spacing:128
    (fun ~xf ->
       fun ~yf ->
         fun ~xi:_ ->
           fun ~yi:_ ->
...
let apply_cavern_entrance args ~tube_depth  ~x  ~z  =
  (let top = Site_templates.cavern_entrance in
   ...
   let y =
     (max_height_within args ~minx:(minx + x) ~maxx:(maxx + x)
   ...
   (let tube_height = Minecraft_template.height tube in
    ...
    (let y = (y - (tube_sections * tube_height)) - base_height in
     ...
     (let (minx, maxx) = base.bounds_x in
      ...
      Building.stair_foundation args ~minx:(minx + x) ~maxx:(maxx + x) ~y
        ~minz:(minz + z) ~maxz:(maxz + z)))) : unit)
let apply_region sites (args : Minecraft_converter.region_args) =
  (Sparse_grid.iter sites.Point_cloud.points
     (fun _ ->
        fun Point_cloud.{ px = x; py = z; value = site }  ->
          ...
          then
            match site with
            | ((Some
                (((Cavern_entrance (tube_depth))[@explicit_arity ])))
                [@explicit_arity ]) ->
