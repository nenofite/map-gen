type t = {
    ...
    blocks: Bigarray.Genarray.t(int, Bigarray.int8_unsigned_elt, Bigarray.c_layout),
    mutable entities: list(Entity.t),
};

let block_per_chunk_side = 16;
let block_per_section_vertical = 16;
let section_volume = block_per_chunk_side * block_per_chunk_side * block_per_section_vertical;
let section_per_chunk_vertical = 16;
let block_per_chunk_vertical = block_per_section_vertical * section_per_chunk_vertical;
let chunk_per_region_side = 32;
let block_per_region_side = chunk_per_region_side * block_per_chunk_side;
let block_per_region_vertical = section_per_chunk_vertical * block_per_chunk_vertical;
let num_channels = 2;

let create = (~rx, ~rz) => {
    open Bigarray;
    let blocks = Genarray.create(Int8_unsigned, C_layout, [|block_per_region_side, block_per_region_side, block_per_region_vertical, num_channels|]);
...
let is_within = (~x, ~y, ~z, r) => {
    ...
    0 <= x && x < block_per_region_side && 0 <= y && y < block_per_region_vertical && 0 <= z && z < block_per_region_side;
};
...
let assert_and_localize = (~x,~y,~z,r) => {
    if (is_within(~x,~y,~z,r)) {
        ...
        let msg = Printf.sprintf("coords outside of %d, %d region: %d, %d, %d", r.rx, r.rz, x, y, z);
